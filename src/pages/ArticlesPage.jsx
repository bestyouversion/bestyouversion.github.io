...
import { HypnotherapyIllustration, ThinkingIllustration, FreedomIllustration } from '../components/Illustrations'
import Navbar from '../components/Navbar'
import Footer from '../components/Footer'

const ILLUSTRATION_MAP = {
  hypnotherapy: HypnotherapyIllustration,
  thinking: ThinkingIllustration,
  freedom: FreedomIllustration,
}

function ArticleIllustration({ type, height = 200 }) {
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      <section style={{ padding: '140px 32px 80px', maxWidth: 960, margin: '0 auto' }}>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.3em', color: 'var(--accent)', marginBottom: 16 }}>Insights</div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: 1.15, color: 'var(--text-primary)', margin: '0 0 56px' }}>Articles</h1>
          ...
          {ARTICLES.map(article => (
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--accent-border)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'var(--shadow-card)' }}>
              ...
              <div style={{ padding: '24px' }}>
                ...
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.6, color: 'var(--text-muted)', margin: 0 }}>{article.excerpt}</p>
                <div style={{ marginTop: 16, fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--accent)' }}>Read more →</div>
              </div>
            </Link>
          ))}
        </div>
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      <article style={{ padding: '140px 32px 80px', maxWidth: 700, margin: '0 auto' }}>
        <Link to="/articles" style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 32, display: 'inline-block' }}>← Back to Articles</Link>
        <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 'clamp(28px, 4vw, 42px)', lineHeight: 1.2, color: 'var(--text-primary)', margin: '0 0 32px' }}>{article.title}</h1>

        <div style={{ borderRadius: 'var(--radius-xl)', overflow: 'hidden', marginBottom: 40, border: '1px solid var(--accent-border)' }}>
          <ArticleIllustration type={article.illustration} height={300} />
        </div>
